...
import {
  ...
  DelUserEmailRequest,
  DelUserPhoneRequest,
  DelUserAddressRequest,
  DelUserExperienceRequest,
  DelUserEducationRequest,
  DelUserSkillRequest,
  GetLineAddressRequest,
  GetUserCityRequest,
...
export default function Setting() {
  ...
  const { role } = useSelector((state) => state.userSettingState);
  const { user } = useSelector((state) => state.userSettingState);
  const { email } = useSelector((state) => state.userSettingState);
  const { phone } = useSelector((state) => state.userSettingState);
  const { address } = useSelector((state) => state.userSettingState);
  const { education } = useSelector((state) => state.userSettingState);
  const { experience } = useSelector((state) => state.userSettingState);
  const { skill } = useSelector((state) => state.userSettingState);
  ...
  useEffect(() => {
    dispatch(GetUserRoleRequest(1));
    dispatch(GetUserRequest(1));
    dispatch(GetUserEmailRequest(1));
    dispatch(GetUserPhoneRequest(1));
    dispatch(GetUserAddressRequest(1));
    dispatch(GetUserEducationRequest(1));
    dispatch(GetUserExperienceRequest(1));
    dispatch(GetUserSkillRequest(1));
    dispatch(GetUserCityRequest());
  }, [refresh]);
  const onDeletedEmail = async (id, email) => {
    dispatch(DelUserEmailRequest(id, email));
  };
  const onDeletedAddress = async (id, address) => {
    dispatch(DelUserAddressRequest(id, address));
  };
  const onDeletedPhone = async (id, phone) => {
    dispatch(DelUserPhoneRequest(id, phone));
  };
  const onDeletedEducation = async (id, education) => {
    dispatch(DelUserEducationRequest(id, education));
  };
  const onDeletedExperience = async (id, experiences) => {
    dispatch(DelUserExperienceRequest(id, experiences));
  };
  const onDeletedSkill = async (id, skill) => {
    dispatch(DelUserSkillRequest(id, skill));
  };
                            ...
                            >
                              <div className="w-3/6">
                                ...
                                <div className="flex mb-3 gap-3 mt-1 ">
                                  <p>City :</p>
                                  <p>{address.etad_addr.addr_city.city_name}</p>
                                </div>
                              </div>
                                  ...
                                  <button
                                    onClick={() => {
                                      if (
                                        ...
                                        onDeletedAddress(
                                          address.etad_entity_id,
                                          address.etad_addr_id
                                        );
                                      }
                                    }}
                                  >
