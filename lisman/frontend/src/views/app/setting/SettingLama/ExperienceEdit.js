...
export default function ExperienceEdit(props) {
  ...
  const { experiences } = useSelector((state) => state.userSettingState);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(GetUserCityRequest());
    dispatch(GetOneUserExperienceRequest(props.id, props.idNext));
  }, []);
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      usex_id: experiences.usex_id,
      usex_entity_id: experiences.usex_entity_id,
      usex_title: experiences.usex_title,
      usex_profile_headline: experiences.usex_profile_headline,
      usex_company_name: experiences.usex_company_name,
      usex_city_id: experiences.usex_city_id,
      usex_start_date: experiences.usex_start_date,
      usex_end_date: experiences.usex_end_date,
      usex_industry: experiences.usex_industry,
      usex_employment_type: experiences.usex_employment_type,
      usex_experience_type: experiences.usex_experience_type,
    },
    ...
    onSubmit: async (values) => {
      ...
      props.closeAdd();
      dispatch(EditExperienceRequest(payload));
      props.refresh();
              ...
              <form onSubmit={formik.handleSubmit}>
                <div>
                  ...
                  <input
                    ...
                    name="usex_id"
                    id="usex_id"
                    defaultValue={formik.values.usex_id}
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                  />
                  ...
                  <div className="mt-3 ml-16 flex items-center ">
                    <label htmlFor="usex_city_id" className="mr-[2.5rem] ml-10">
                      City
                    </label>
                    <select
                  ...
                  "
                    >
                      ...
                      {Array.isArray(city)
                        ? city &&
                          city.map((city, index) => (
                            <option key={index} value={city.city_id} selected={(city.city_id === props.city_id) ? true : false}>
                              {city.city_name}
                            </option>
                          ))
                        : null}
