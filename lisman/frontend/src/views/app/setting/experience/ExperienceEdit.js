...
import {
  ...
  GetCityRequest,
  GetOneExperienceRequest,
} from "../../../../redux-saga/actions/MasterSetting";

export default function ExperienceEdit(props) {
  const { experience, city } = useSelector((state) => state.masterSettingState);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(GetCityRequest());
    dispatch(GetOneExperienceRequest(props.id, props.idNext));
  }, []);

  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      usex_id: experience.usex_id,
      usex_entity_id: experience.usex_entity_id,
      usex_title: experience.usex_title,
      usex_profile_headline: experience.usex_profile_headline,
      usex_company_name: experience.usex_company_name,
      usex_city_id: experience.usex_city_id,
      usex_start_date: experience.usex_start_date,
      usex_end_date: experience.usex_end_date,
      usex_industry: experience.usex_industry,
      usex_employment_type: experience.usex_employment_type,
      usex_experience_type: experience.usex_experience_type,
    },
    ...
    onSubmit: async (values) => {
      const payload = {
        usex_id: parseInt(values.usex_id),
